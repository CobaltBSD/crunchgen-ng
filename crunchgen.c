...
fillin_program(prog_t * p)
{
	...
	if (!p->srcdir && verbose)
		fprintf(stderr, "%s: %s: warning: could not find source directory.\n",
		    infilename, p->name);
	if (!p->objs && verbose) //FIXME: this warning shows regardless of if there's a problem or not
...
top_makefile_rules(FILE * outmk)
{
	...
	fprintf(outmk, "CFLAGS := -no-pie -Oz -fno-stack-protector -fno-unwind-tables -fno-asynchronous-unwind-tables\n");
	fprintf(outmk, "LDFLAGS := -no-pie\n");
	fprintf(outmk, "STRIP ?= strip\n");
	fprintf(outmk, "CC ?= gcc\n");
	fprintf(outmk, "LINK ?= ld ${LDFLAGS} -z undefs -r\n");
	fprintf(outmk, "LIBS :=");
...
*/
	fprintf(outmk, "%s: %s.o $(CRUNCHED_OBJS) ${CLIB}\n",
	    execfname, execfname);
	fprintf(outmk, "\t$(CC) -static -L. ${LDFLAGS} -o %s %s.o $(CRUNCHED_OBJS) $(LIBS)\n",
	    execfname, execfname);
...
*/
	...
	//    elf_mangle ? "-M" : "",
	//    elf_names ? "" : "_", p->ident);
	fprintf(outmk, "\tcrunchgen -h %s -k _crunched_%s_stub $@\n",
	    elf_mangle ? "-M" : "",
	    p->ident);
}
